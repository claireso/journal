...
export const update_photo = (id, fields) => `UPDATE photos SET ${fields} WHERE id=${id} RETURNING *`

export const get_photo = (id, fields = '*') => `SELECT ${fields} FROM photos WHERE id=${id}`
...
export const delete_subscription = (id) => `DELETE FROM subscriptions WHERE id=${id}`

export const find_subscription = (id, fields = '*') => `SELECT ${fields} FROM subscriptions WHERE id=${id}`
...
export const find_user_by_cid = (cid, fields = '*') => `SELECT ${fields} FROM users WHERE cid='${cid}'`
