...
router.post('/photos/new', upload.single('file'), catchErrors(async (req, res, next) => {
...
// EDIT PHOTO
router.get('/photos/:id(\\d+)/edit', catchErrors(async (req, res, next) => {
  const { id } = req.params
...
// DELETE PHOTO
router.get('/photos/:id(\\d+)/delete', catchErrors(async (req, res, next) => {
  const { id } = req.params
