...
describe('<Edit />', () => {
  ...
  test('should fill form with photo', async () => {
    ...
    expect(container.querySelector('input[name="description"]').value).toEqual(
      'Février 2019'
    )
    expect(container.querySelector('img').src).toEqual(
      'http://localhost/uploads/01d2y7jt2j24dv0s82m9xq729d.jpg'
    )
  ...
  test('should not submit form', async () => {
    PhotosReducer.usePhotosReducer = () => [
      { ...INITIAL_STATE, status: 'pending', single: single },
      {}
    ]
